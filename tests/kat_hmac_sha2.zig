...
test "HMAC-SHA-256 RFC 4231 #2: key=\"Jefe\"" {
    const got = try HmacSha256.auth("Jefe", "what do ya want for nothing?");
...
test "HMAC-SHA-256 RFC 4231 #4: keyed 25, data=0xcd*50" {
    ...
    const got = try HmacSha256.auth(&key, &data);
...
test "HMAC-SHA-384 RFC 4231 #1" {
    const key = @as([20]u8, @splat(0x0b));
    const got = try HmacSha384.auth(&key, "Hi There");
...
test "HMAC-SHA-512 RFC 4231 #1" {
    const key = @as([20]u8, @splat(0x0b));
    const got = try HmacSha512.auth(&key, "Hi There");
...
test "HMAC-SHA-256 streaming matches one-shot" {
    ...
    try h.update("hello, ");
    try h.update("world!");
    const streamed = try h.finalDigest();
    const oneshot = try HmacSha256.auth(key, "hello, world!");
